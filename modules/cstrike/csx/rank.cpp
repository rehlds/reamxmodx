...
static cell AMX_NATIVE_CALL get_user_rank(AMX *amx, cell *params) /* 1 param */
{
	...
	if (pPlayer->rank) return pPlayer->rank->getPosition();
...
static cell AMX_NATIVE_CALL set_stats(AMX *amx, cell *params) /* 3 param */
{
	...
	if (a)
	{
		...
		if (cpStats[0] >= 0) (*a).kills = cpStats[0];
		
		if (cpStats[1] >= 0) (*a).deaths = cpStats[1];
		
		if (cpStats[2] >= 0) (*a).hs = cpStats[2];
		
		if (cpStats[3] >= 0) (*a).tks = cpStats[3];
		
		if (cpStats[4] >= 0) (*a).shots = cpStats[4];
		
		if (cpStats[5] >= 0) (*a).hits = cpStats[5];
...
static cell AMX_NATIVE_CALL set_stats2(AMX *amx, cell *params) /* 2 param */
{
	...
	if (a)
	{
		...
		if (cpStats[0] >= 0) (*a).bDefusions = cpStats[0];
		
		if (cpStats[1] >= 0) (*a).bDefused = cpStats[1];
		
		if (cpStats[2] >= 0) (*a).bPlants = cpStats[2];
		
		if (cpStats[3] >= 0) (*a).bExplosions = cpStats[3];
...
static cell AMX_NATIVE_CALL set_user_stats(AMX *amx, cell *params) /* 3 param */
{
	...
	if (pPlayer->rank)
	{
		...
		if (cpStats[0] >= 0) pPlayer->rank->kills = cpStats[0];
		
		if (cpStats[1] >= 0) pPlayer->rank->deaths = cpStats[1];
		
		if (cpStats[2] >= 0) pPlayer->rank->hs = cpStats[2];
		
		if (cpStats[3] >= 0) pPlayer->rank->tks = cpStats[3];
		
		if (cpStats[4] >= 0) pPlayer->rank->shots = cpStats[4];
		
		if (cpStats[5] >= 0) pPlayer->rank->hits = cpStats[5];
...
static cell AMX_NATIVE_CALL set_user_stats2(AMX *amx, cell *params) /* 2 param */
{
	...
	if (pPlayer->rank)
	{
		...
		if (cpStats[0] >= 0) pPlayer->rank->bDefusions = cpStats[0];
		
		if (cpStats[1] >= 0) pPlayer->rank->bDefused = cpStats[1];
		
		if (cpStats[2] >= 0) pPlayer->rank->bPlants = cpStats[2];
		
		if (cpStats[3] >= 0) pPlayer->rank->bExplosions = cpStats[3];
...
static cell AMX_NATIVE_CALL push_stats(AMX *amx, cell *params) /* 5 param */
{
	...
	if (cpStats[0] >= 0) temp->kills = cpStats[0];
	
	if (cpStats[1] >= 0) temp->deaths = cpStats[1];
	
	if (cpStats[2] >= 0) temp->hs = cpStats[2];
	
	if (cpStats[3] >= 0) temp->tks = cpStats[3];
	
	if (cpStats[4] >= 0) temp->shots = cpStats[4];
	
	if (cpStats[5] >= 0) temp->hits = cpStats[5];
...
static cell AMX_NATIVE_CALL remove_stats(AMX *amx, cell *params) /* 1 param */
{
	...
	if (a)
	{
		for (int i = 1; i <= gpGlobals->maxClients; i++)
		{
			...
			if (players[i].rank && players[i].rank->getPosition() == index)
			{
